        ...
        <div class="testi-left">
            <div class="testi-wrap">
                <div class="slider" id="tSlider">
                            ...
                            <div class="t-item" id="testi-{{ $loop->iteration }}">
                                ...
                                @if (isset($r->rating))
                                    <div class="t-stars" aria-label="Rating {{ (int) $r->rating }} dari 5">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <i
                                                class="{{ $i <= (int) $r->rating ? 'fa-solid' : 'fa-regular' }} fa-star"></i>
                                        @endfor
                                    </div>
                                @endif
                    ...
                    <div class="t-nav">
                        <button class="t-btn" data-dir="-1" aria-label="Prev"><i
                                class="fa fa-chevron-left"></i></button>
                        <button class="t-btn" data-dir="1" aria-label="Next"><i
                ...
                @if (isset($heroes) && $heroes->isNotEmpty())
                    <div class="testi-media-grid">
                        @foreach ($heroes as $hero)
                            <img src="{{ asset('public/storage/' . $hero->image) }}" alt="Studio Prewed Team"
                                loading="lazy"
                                onerror="this.onerror=null;this.src='{{ asset('asset/IMGhome/bg1.jpg') }}'">
                        @endforeach
                    </div>
                @else
                    <img src="{{ asset('asset/IMGhome/bg1.jpg') }}" alt="Studio Prewed Team" loading="lazy">
                @endif
            </div>
        </div>
    </div>
...
<div class="reviews-cta" style="display:flex; gap:12px; justify-content:center; margin:24px 0 8px;">
    <button id="btnOpenReviews" type="button" class="btn-reviews" aria-controls="reviewsSection"
        aria-expanded="false">Selengkapnya</button>
    <button id="btnHideReviews" type="button" class="btn-reviews is-ghost" aria-controls="reviewsSection"
        hidden>Sembunyikan</button>
</div>
