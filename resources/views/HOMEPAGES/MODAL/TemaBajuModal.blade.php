...
@foreach ($temasArr as $i => $t)
    ...
    <div class="pkg-modal" id="temaModal-{{ $t->id }}" role="dialog" aria-hidden="true">
        ...
        <div class="pkg-modal__panel" role="document" aria-label="Detail tema {{ $t->nama }}">
            ...
            <button type="button" class="nav-btn prev" data-tema-nav data-target="#temaModal-{{ $prevId }}"
                aria-label="Tema sebelumnya">
                <i class="fas fa-chevron-left"></i>
            </button>

            <button type="button" class="nav-btn next" data-tema-nav data-target="#temaModal-{{ $nextId }}"
            ...
            <div class="pkg-modal__body">
                ...
                <div class="pkg-modal__media">
                    <div class="tbm-gallery">
                        <div class="tbm-main">
                            <img id="tbmMain-{{ $t->id }}" src="{{ $imgUrls[0] }}"
                                alt="Foto {{ $t->nama }}">
                        </div>
                        ...
                        @if (count($imgUrls) > 1)
                            <div class="tbm-thumbs" role="listbox" aria-label="Pilih foto {{ $t->nama }}">
                                @foreach ($imgUrls as $k => $src)
                                    <button type="button" class="tbm-thumb {{ $k === 0 ? 'is-active' : '' }}"
                                        data-main="#tbmMain-{{ $t->id }}" data-src="{{ $src }}"
                                        aria-label="Foto {{ $k + 1 }}">
                                        <img src="{{ $src }}" alt="Thumbnail {{ $k + 1 }}"
                                            loading="lazy">
                                    </button>
                                @endforeach
                            </div>
